...
// Scans the configured giveaway channel (config.giveawayCheckChannelId) for
// a message that both mentions the ticket owner and contains the amount
// they say they won. Returns { configured, found, message } where message
// is { url, channelId, messageId, createdTimestamp } for the best match.
async function findGiveawayWin(guild, ownerId, amount) {
  const channelId = config.giveawayCheckChannelId;
  ...
  const limit = Math.max(50, Math.min(Number(config.giveawayScanMessageLimit || 500), 5000));
  const matches = [];
